...
## Palette picker for the gateway theme. Populates the controller OptionButton with
## the gateway palettes found on disk and persists the chosen slug to
## [gateway]/palette. Mirrors setting_language.gd. The gateway live-applies the
## change via ClientState.settings.setting_changed, so no relaunch is needed.


func _ready() -> void:
	assert(is_instance_valid(controller) and controller is OptionButton)
	var options: OptionButton = controller
	for palette: StringName in GatewayTheme.list_palettes():
